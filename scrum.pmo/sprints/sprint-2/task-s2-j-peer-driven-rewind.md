...
- Tester (T-REWIND-DRIVE): **✅ SAFE SURFACE PASS 6/6** (dev `test/test.rewind-drive`) — RD-NOT-IDLE-REFUSE (rc3, 0 /rewind posted), RD-BUSY-REFUSE (rc3, no interrupt), RD-SELF-REFUSE [S], RD-AUTH-REFUSE [S] (Tron-auth gate precedes keystroke, line 15<30), RD-FAILSAFE-SRC [S] (unverified→Escape-out→idle-unharmed→rc2, never blind re-Enter), RD-POKE-GUARD/CLEAR/REAP [S] (DURING_REWIND set-before + clear-every-exit + reap-rc0-only). **HELD FOR AUTHORIZATION** — the 3 DESTRUCTIVE live cells (d idle-authorized picker drive / e select-stall→agent-unharmed [critical] / f concurrent-send-held) irreversibly rewind a REAL agent; NOT run autonomously. Need explicit Tron/PO auth + a designated sacrificial peer + a real picker capture to calibrate the signature regexes. Ready to run d/e/f on go+target.
