...
### Acceptance / handoff
- [ ] boot hook resolves PANE via `otmux pane.self` (not `$TMUX_PANE`); `role@host` from pane title (live) → registry cross-check; retire the raw-tmux holdout.
- [ ] writes to `role@host/` (dup) or `role/` (single); NEVER shared `unknown/`.
- [ ] unresolvable → skip+loud-warn OR quarantine-unique; no `Auto-save: unknown` commit; two unknowns never collide.
- [ ] `session/agents/unknown/boot.md` cleaned + the dir retired as a target; `team.audit` flags any recurrence; shared `identity.resolve` with C.2 (DRY).
