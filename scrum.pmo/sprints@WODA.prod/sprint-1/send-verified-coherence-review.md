...
- **✅ RESOLVED — no code change needed (oosh-expert@ooshTeam:0.3, 2026-07-15).** Measured origin/dev before implementing: the **2nd fix-option in this very line already landed** via **g.4 `6213ad6`** (on origin/dev). `origin/dev:claudeCode` `process.find:965` now trims the tty (`tty=$(printf '%s' "$tty" | tr -d '[:space:]')`) — the `this`-dispatch stray-newline that made `$2==tty` miss a bash-PARENT claude is gone. Chain end-to-end is now correct: `send.smart` → `isClaudeCode` (otmux:1742 bash-branch → `claudeCode process.running`) → `process.find` (g.4-fixed) → **bash-parent claude=claude**. So send.smart's existing `isClaudeCode` call IS correctly wired (hardening the root beats rewiring to a separate reader — same net effect, less surface). Proof: T-KIND-CLASSIFY 12/12; g.4 msg "verified on PO pane 0.0". **Remaining = tester go-through TC-5** (live bash-parent claude classifies CLAUDE), NOT reimplementation.
...
- **✅ RESOLVED — no code change needed (oosh-expert@ooshTeam:0.3, 2026-07-15).** Measured origin/dev: the suppression is **already implemented** in `otmux.send` Case-2 (`origin/dev:otmux:2054-2073`): after `private.otmux.send.smart` (which commits via send.verified→Enter), a trailing **Enter is SKIPPED** (`debug.log "Case-2 skipped a redundant trailing Enter … send.smart already committed"`), while a trailing **NON-Enter** key (y/Down/…) is still sent (`otmux.send.raw "$target" "$last"`) — exactly the contract. This is unconditional (correct: send.smart submits on BOTH shell and claude, per tester TC-8 shell proof = exactly 1 Enter). **Remaining = tester go-through TC-8 on a CLAUDE target** (confirm exactly-one-Enter, no re-submit), NOT reimplementation.
