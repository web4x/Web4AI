...
- Expert (c2 fix + commit): **DONE** `f13f35d` (crash guard) + `d83907b` (`;` fix — RC=0 on found completion, extract first param from signature). PO-verified live: `otmux attach [Tab][Tab]` → session names (TRONinterface, baseTeam, iphone, ooshTeam, prod, robbinTeam2). Zero errors. `__test_` filtered. Root cause was debug.log RC=1 leaking → caller fell through to `;` fallback.
