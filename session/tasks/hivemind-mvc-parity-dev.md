...
- **PO VERIFIED (2026-06-24)**: WODA.prod HEAD=f74c20a, `git branch --contains 80fdbd8`→dev (merge carried macos.latest tip), d45031a on dev, `config.validate`=4 live on WODA.prod (was 0). dev==macos.latest MVC count (25 both, 3-term grep). My earlier "38 vs 25" alarm was a grep-pattern inconsistency (4 terms vs 3) — false. Parity achieved. Remaining: tester formal method-surface diff as QA sign-off (after T-ENV-PURE).
