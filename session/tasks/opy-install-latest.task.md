...
### ✅ EXPERT — FIXED + LIVE (oosh-expert@ooshTeam:0.3, 2026-07-18) `7282bd7`
Clean worktree → push origin/test/mcdonges.latest (`aa2c4f5..7282bd7`, opy-only) → oosh-po heads-up (gate chain) → **live ff-deploy `/root/oosh`** (clean ff). `opy.deps` now ensures the base C toolchain FIRST, **independent of the module-probe**: `if ! command -v cc || ! command -v make → toolchain="build-essential"`; the early "nothing to install" return is gated on `[ -z "$missing" ] && [ -z "$toolchain" ]` (so a box with all modules but no compiler STILL gets build-essential); `pkgs` is seeded with `$toolchain` and prepended before the header packages. Probe verified on this live box: **`cc` ABSENT → build-essential triggers** (make present, but the OR fires on cc) — matches the GAP. `bash -n` clean; live opy:222-223 carry the probe. Did NOT run the real build (apt build-essential + multi-min CPython compile) — that's the tester's true one-command end-to-end proof on a gcc-less box. Toolchain name `build-essential` is Debian/apt (consistent with the existing Debian `-dev` header mapping; cross-distro pkg naming is a separate concern, not THE GAP). Dual-link: `7282bd7` (once.sh:test/mcdonges.latest + live /root/oosh) | `/root/oosh/opy:212` (opy.deps). → tester **`opy install` no-arg BUILDS end-to-end on a box without gcc**.

---
## 🟡 GAP 2 — compiles now, but `make install` fails at ensurepip/pip (opy1@WODA.prod, 2026-07-18)
Next run got MUCH farther: build-essential fix landed → full toolchain (gcc-9/g++/build-essential/libc6-dev) installed → Python-3.14.6 downloaded, configured, **COMPILED** (compiler gap CLOSED). But `make install` FAILS at `Makefile:2500` — ensurepip's pip-bootstrap: `pip install --no-index --find-links /tmp/… --root / --upgrade pip` → `CalledProcessError: exit 1`. The pane shows only the ensurepip WRAPPER traceback; the real pip error is in `/tmp/python-build.20260718145700.2016873.log`.
→ **oosh-expert**: READ the build log, find the pip-bootstrap ROOT. Candidates: (a) pyenv/python-build (2.8.0) too old for 3.14.6 → self-care: ensure pyenv is fresh enough for the latest; (b) a platform dep pip/ssl/hashlib needs on Ubuntu 20.04 focal (old OpenSSL 1.1.1 vs 3.14 requirements) → add to opy.deps; (c) if 3.14.6 is genuinely unbuildable on focal, `private.opy.latest` should pick the latest BUILDABLE stable (fall back to newest that compiles). Close the gap so `opy install` one-commands a WORKING, importable python. Tester: `opy install` no-arg → `python -c 'print(1)'` works. PO gate → Tron.

### ✅ EXPERT — ROOT FOUND (measured the log, not the guesses) + FIXED + LIVE (oosh-expert@ooshTeam:0.3, 2026-07-18) `1535d58`
**Read `/tmp/python-build.20260718145700.2016873.log` (6507 lines).** Real error (line 6464/6471/6507): `ModuleNotFoundError: No module named 'zlib'` → `zipimport.ZipImportError: can't decompress data; zlib not available` → `make: *** [Makefile:2500: install] Error 1`. **NONE of the guessed candidates** (pyenv-too-old / OpenSSL / unbuildable — 3.14.6 downloaded, configured, compiled FINE). **True root: `opy.deps`'s module-probe fell back to the SYSTEM python** (`private.opy.pyenv which python` → `/usr/bin/python3`, which HAS zlib) → zlib seen as "present" → **`zlib1g-dev` SKIPPED** → 3.14.6 compiled WITHOUT zlib → pip bootstrap failed. Confirmed by measurement: `/usr/bin/python3` imports zlib; `dpkg -l zlib1g-dev` = NOT installed. The probe answers the wrong question — "does some python have module X" ≠ "is the -dev HEADER installed to COMPILE a new python with X."
**FIX:** `opy.deps` now probes modules ONLY against the **target version's own python** (valid: a re-check of an already-built version); a **FRESH install (target not built) installs the FULL header set** — never trusts a different python's modules for a build-header decision. Combined with GAP 1 (build-essential), a fresh 3.14.6 build now gets full toolchain + all `-dev` headers incl. `zlib1g-dev` → zlib present → pip bootstraps → importable python. Flow: worktree→push→heads-up→live ff `/root/oosh` (opy-only, clean). `bash -n` clean; live opy:233-234. Did NOT run the real multi-min build (tester's end-to-end `opy install`→`python -c 'print(1)'` proof). Dual-link: `1535d58` (once.sh:test/mcdonges.latest + live /root/oosh) | `/root/oosh/opy:212` (opy.deps). → tester **`opy install` no-arg → importable python**.

## 🟢 END-TO-END GATE — last 2 self-care gaps (oosh-po real ubuntu:24.04 build, 2026-07-20)
The real bare-box end-to-end build (the compile that had never run) proved: with **git + curl** supplied manually, unmodified `opy install` built Python 3.14.6 end-to-end + all modules import OK. The only 2 remaining self-heals:
- **GAP-A**: `private.opy.installPyenv` (opy:110) only ERRORED on missing git → must `pkgInstall git` first (self-care, like build-essential).
- **GAP-B**: `opy.deps` had toolchain+headers but NO downloader → pyenv download fails ("please install aria2c/curl/wget") → add curl.

### ✅ EXPERT — FIXED + LIVE (oosh-expert@ooshTeam:0.3, 2026-07-20) `eb42502`
- **GAP-A**: `installPyenv` (opy:112) → `command -v git || private.opy.pkgInstall git || error` — installs git instead of erroring. Ordering is sound: this runs in the constructor's `ensurePyenv` BEFORE the clone, so git is present when the clone needs it.
- **GAP-B**: `opy.deps` adds a **downloader probe** (opy:229-232): `if ! curl && ! wget && ! aria2c → downloader="curl"`; `pkgs` seeded with `$toolchain $downloader`; the early "nothing to install" return gated on `[ -z missing ] && [ -z toolchain ] && [ -z downloader ]`. curl installs in `opy.deps` BEFORE `pyenv install` downloads — correct order.
- Flow: clean worktree → push origin/test/mcdonges.latest (`a27e3b2..eb42502`) → oosh-po heads-up → live ff `/root/oosh` (opy-only, clean). `bash -n` clean; live opy:112/232/263. Probes verified on this box (git/curl/wget present → correctly no-trigger; bare box → both install). Did NOT re-run the multi-min build — tester's end-to-end GREEN re-run is the proof.
- **⚠ Stray (NOT mine)**: live `/root/oosh/otmux` has an UNCOMMITTED scratch comment ("#### lestes now the workingfile on Head") — someone hand-edited the live tree. Harmless (a comment) + untouched by my opy ff; flagging for whoever owns it to commit/discard (I did not create it, did not revert it).
- Dual-link: `eb42502` (once.sh:test/mcdonges.latest + live /root/oosh) | `/root/oosh/opy` (installPyenv:112, opy.deps downloader:232). → tester **re-run END-TO-END gate → GREEN**.

---
## ✅✅ DONE — END-TO-END RE-GATE GREEN (oosh-po@WODA.prod, 2026-07-20)
**Fresh bare ubuntu:24.04, ZERO manual supplies, unmodified `opy install` (no-arg):** git self-installs → curl self-installs → build-essential + headers → Python 3.14.6 built + activated → `import zlib/ssl/ctypes/sqlite3/bz2/lzma/readline` OK → exit 0. **No third gap.** The full self-care chain (ensure-pyenv + env-fix + build-essential + zlib/full-headers + git + downloader) one-commands a WORKING importable python on a bare box — Tron's requirement met end-to-end.
**Landed fixes (mcdonges lineage, all live on /root/oosh):** df95a02 latest-resolver+activate · 19d8d52 auto-install pyenv · aa2c4f5 deps env-prefix · 7282bd7 build-essential toolchain · 1535d58 full-header set (zlib root) · eb42502 git + curl downloader. Tester: T-OPY-INSTALL-LATEST 13/13 GREEN + real bare-box END-TO-END GREEN. **TASK CLOSED.**
