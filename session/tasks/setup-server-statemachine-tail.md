...
- Expert (fixes + commits + state reached): **IN PROGRESS.**
  ...
  - **ROOT .bashrc WIRING — `bee01a1` (PO go: in-chain SETUP_SERVER state).** Wired root's `~/.bashrc` with `templates/user/bashrcTemplate` as a deterministic state step inside `private.check.root.installation.done` (state 33) — NOT a post-install side-step. Idempotent (skips if already OOSH-wired), backs up the original to `.bashrc.pre-oosh`. A direct-root install now ends with the OOSH bootstrap in root's bashrc, so `env -i bash` becomes a working OOSH shell (the `user.oosh.install` path only runs for SUDO_USER≠root, which a direct-root install never hits). **Clean-boot verify re-install IN PROGRESS** — checking `env -i bash` → OOSH_MODE/OOSH_DIR set, `oo` on PATH, config valid. If green → hand to tester for Step 4+5.
