...
- Expert (fixes + commits + state reached): **IN PROGRESS.**
  ...
  - **ROOT .bashrc WIRING — `bee01a1` (PO go: in-chain SETUP_SERVER state).** Wired root's `~/.bashrc` with `templates/user/bashrcTemplate` as a deterministic state step inside `private.check.root.installation.done` (state 33) — NOT a post-install side-step. Idempotent (skips if already OOSH-wired), backs up the original to `.bashrc.pre-oosh`.
  - **✅✅ S-B COMPLETE — CLEAN BOOT GREEN on pristine u24.** Final verify re-install: state=99 `finished`, root `.bashrc` 15 OOSH lines. **`env -i bash` on u24**: `OOSH_MODE=dev`, `OOSH_DIR=…/dev`, `oo` ON PATH, `config validate` passes, **ZERO stderr errors**. config pure (20 exports/0 source); `~/config`→sharedConfig + `~/oosh`→dev symlinks correct. The full fresh dev install on a box that never had old pollution now boots clean as an OOSH shell. **→ HANDING TO TESTER (S-C) for formal Step 4 + Step 5 (team.push).** ~11 fixes total: BUG5 2b68265 · method-name d546947 · BUG2 044dc75 · BUG1+3 edbbabc · BUG6 376020e · 33-62 batch ffb38c9 · driver-loop 278d5a7 · root-bashrc bee01a1 · +5 gate install-transport (4397ac2/8a3c02d/99fb694/9a87d34).
