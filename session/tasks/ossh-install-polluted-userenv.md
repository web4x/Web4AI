...
- [ ] **`this` self-heals on boot (never runs broken, never asks for help)**: with a polluted env, boot AUTO-HEALS to a valid object — it does NOT print "run config repair" (objects never fail / never ask the user to fix them; they fix themselves). No empty/RC=0 silence either — it's valid because it healed.
...
## ★★★ GOVERNING PRINCIPLE (Tron 2026-06-26) — init IS the constructor; this is the OBJECT-ORIENTED shell
**`init` ALWAYS makes the object fully operational, consistent, and safe — every invocation, by definition.** OOSH is OOP: `scriptname.start()` / `this.init` / `config.init` ARE constructors. A constructor's contract is: after it runs, the object is VALID — fully operational, internally consistent, safe to use. There is NO "loaded-but-broken" state for a properly constructed object.
**This SUPERSEDES the "bolt-on repair" framing.** Self-repair is NOT a separate `config repair` command you remember to run — it is simply what a correct `init` does. init is **idempotent and self-healing by design**: call it on a fresh box or a born-broken one (empty OOSH_DIR, polluted env, u20) and it ALWAYS yields a valid object, with NO config loss. Rule B ("any script reinit clean") = every script's init guarantees this; "repair" is just init invoked again.
**Concretely**: `config.init` / `this.init` must, on EVERY call: resolve fundamentals (OOSH_DIR/CONFIG_PATH/OOSH_MODE) from the canonical source (BASH_SOURCE — the script's own location), preserve all existing user state, emit pure-state env (state + `source *.env` only), validate — and leave a fully operational object. It ALWAYS can (BASH_SOURCE is always present) → it ALWAYS self-heals → it NEVER fails. No failure path, no half-constructed object. #10/#11/repair-cant-heal all dissolve into this: a constructor that always builds a valid object cannot ship a broken user.env.
**Reframe the work**: not "add repair" — make the CONSTRUCTORS (config.init, this.init, every script's .start) honor the constructor contract. Architect: rewrite the self-care principle in `first-principles.md` as "init = constructor guarantee (operational+consistent+safe), idempotent + self-healing + no-loss" and align config.validate/init/this.init specs to it.

## ★★★★ FIRST PRINCIPLE — authoritative text (oosh-po, guardian, 2026-06-26). Architect: place VERBATIM as the FIRST Philosophy bullet in docs/first-principles.md, REPLACING the bolt-on "Self-Care" obligation-3 wording.

> **init() is the constructor — it ALWAYS yields a fully operational, consistent, and safe object.**
> OOSH is the Object-Oriented Shell: `scriptname.start()` / `this.init` / `config.init` are constructors. The constructor contract is absolute — after init runs, the object IS valid: fully operational, internally consistent, safe to use. There is no "loaded-but-broken" state.
...
> - **Objects never fail — they self-heal.** A constructor has NO failure path. Because fundamentals derive from the canonical source (`BASH_SOURCE` — the running script's own location, always present), init can ALWAYS resolve and heal, so it ALWAYS reaches a valid object. There is no "unrecoverable env", no "fail loud", no half-constructed object. Whatever the input state — fresh, polluted, empty, born-broken — init self-heals it to valid and succeeds.

This is THE first principle; #10/#11/repair-cant-heal/self-care all derive from it. Every script's constructor honors it; every script can therefore reinit a broken env clean (Rule B) because that is just calling the constructor.
