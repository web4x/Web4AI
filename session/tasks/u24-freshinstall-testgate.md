...
- Tester (independent verification of steps 4 & 6) — **oosh-tester, 2026-06-28, dev tip bee01a1**:
  - **STEP 4 (clean boot on u24): GREEN ✓** — verified on the live u24 container (port 9024) via ssh:
    - `env -i bash`: **OOSH_MODE=dev**, OOSH_DIR=…/Once.sh/dev (valid in-container; `~/oosh` is a symlink to the container-local dev tree, NOT a host mount), **oo ON PATH** ✓
    - `env -i bash`: **0 error lines** (no `/.local/bin/env`, HOME resolved by `this`) ✓ — BUG1 confirmed on a never-polluted box
    - config: **20 exports / 0 source lines** (pure-state, #6 acceptance) ✓
    - **`config validate` PASS** ✓
    - `config`/`otmux`/`claudeCode` all on PATH at the dev tree ✓
  - **STEP 4 — 2 checks N/A on u24 (missing agent-runtime tooling, NOT OOSH-boot failures):**
    - `claudeCode list` color: 0 ANSI seqs — because **claude-cli is NOT installed on u24** (no sessions to list → nothing to colorize). The color fix is present in the script (`FORCE_COLOR=2`, claudeCode:17; host renders 44 seqs). Not a color-fix failure; just no sessions on u24.
    - `otmux pane.self` / `pane.title CURRENT`: **tmux NOT installed on u24** → cannot exercise. (Both verified GREEN on the host in BUG7 QA.)
  - **STEP 5 (team push): BLOCKED 🔴** — `hiveMind team.push u24` ran to `COMPLETE` but pushed **0 agents** ("ooshShells → u24 (0 agents)", `git pull failed on u24 — continuing`). Root cause: **u24 has NO tmux and NO claude-cli** — both are prerequisites to create agent panes + fork live Claude agents. The `ossh install` provisions the OOSH framework cleanly but does NOT install tmux/claude; team.push does not provision them either. No live agents could be placed → not discoverable via `claudeCode list` on u24.
  - **TESTER VERDICT**: Step 4 clean-boot (the bug-sprint acceptance) is **independently CONFIRMED GREEN** on a fresh box — OOSH boots clean, pure config, zero errors, HOME-discovery works. **FULL GATE still HELD**: Step 5 cannot pass until u24 is provisioned with **tmux + claude-cli** (install-path or team.push responsibility — recommend `ossh install` provision both, or document them as a team.push prereq it installs). Suggest a follow-up: `ossh install` (or `hiveMind team.push`) ensures tmux + claude on the target before forking agents.
