...
1. Confirm you're on `mcdonges.latest`, clean tree (`git -C ~/oosh status --short`). If dirty → report, don't blind-work.
...
5. Commit on `mcdonges.latest`. Report the commit hash + the awk output (proving `-p`, no `-S`) back in this file.
...
- [ ] `git -C ~/oosh log --oneline` shows the port commit on `mcdonges.latest`
...
## REPORT-BACK — oosh-expert (PORTED `b2dd551`, pushed origin/mcdonges.latest)
Mechanic: **`git cherry-pick 7059a36`** — clean auto-merge, no conflict (preserves authorship + ref; NO rebase). Pre-checks: on `mcdonges.latest`, tree clean (only untracked `macos/`, not tracked churn), buggy `-S` confirmed at old line 2889, `git branch --contains 7059a36` = dev-only before port.
...
`git log --oneline -1` → `b2dd551 otmux pane.capture: read VISIBLE screen (-p) not scrollback (-S) …` on `mcdonges.latest`.
...
Commit **b2dd551** on `mcdonges.latest` (cherry-pick of dev 7059a36):
...
Mirror the dev-side gate (#37 tester 1c5a4e8: negative-control fence + local A/B + interior-blank) but run it on **THIS branch (`mcdonges.latest`) / THIS box (MacStudio)** — the port could interact with macos.latest divergence, so re-prove locally:
...
**Commit (my OWN gate):** `a6a98dc` on `mcdonges.latest` (pushed origin) — `test/test.otmux` T-CAPTURE-BRIDGE block (89 insertions). Independent of the expert's `bash -n`/awk self-check.
