...
### ✅ EXPERT — G1 LANDED (oosh-expert@ooshTeam:0.3, 2026-07-17) `93de8ac` on origin/test/mcdonges.latest
Clean worktree (worktree removed); `bash -n` clean; **branch only — NOT live yet** (ff-deploy to `/root/oosh` on Tron go, with an oosh-po heads-up first per the gate chain).
- **ONE canonical `private.otmux.pane.resolve`** — ported the proven `dev` PID-walk (`$$` → `list-panes -a -F '#{pane_pid} #{pane_id}'` awk-match → `ps -o ppid=` up the tree; first ancestor that hosts a pane wins; rc1 if not under tmux). Returns `#{pane_id}`.
- **Defined the missing `otmux.pane.self`** `<?format:%>` (%=pane_id / target=session:win.pane / any tmux format) — this was the live **called-but-undefined** break (hiveMind:2113/2496/2555/8108 + claudeCode:1620 were dispatching to nothing → empty self-ID). + `private.otmux.self.session`, `otmux.pane.self.completion.format`.
- **All self-ID delegates now**: `otmux.current` → `pane.get.target` → `private.otmux.pane.resolve`; `pane.get` no-target default; `fit` + `size.currentSession` (session-resolve); **send-prefix (1806)** repointed to `otmux.pane.self target`. **Killed** the stale "Self-pane resolution MUST use $TMUX_PANE (Tron P0 #3)" directive (superseded — it lies after move/rewind).
- **Gates (all pass on my live smoke):** (1) **static** — zero `$TMUX_PANE` self-ID CODE uses in otmux (remaining hits are explanatory comments documenting the ban); **zero** TMUX_PANE in hiveMind + claudeCode. (2) **poisoned `TMUX_PANE=%999`** — `otmux current`/`pane.self target` STILL returned the REAL pane `ooshTeam:0.3` (resolver ignores the poison = the move/rewind lie is fixed). (3) **definedness** — `otmux pane.self` → `%3` (non-empty; was undefined). Bonus: the resolver confirmed my own pane = `ooshTeam:0.3`, matching this session's UUID→tty→pane measurements (and refuting the stale `$TMUX_PANE=%8→robbinTeam2:0.3` lie).
- Dual-link: `93de8ac` (once.sh:test/mcdonges.latest) | `origin/test/mcdonges.latest:otmux` (`private.otmux.pane.resolve`, `otmux.pane.self`). → tester **T-NO-TMUXPANE extended** (static + poisoned-%999 + definedness) → PO gate → Tron → ff-deploy live.

### ✅ TESTER — T-NO-TMUXPANE EXTENDED VERIFIED GREEN 7/7 (oosh-tester@WODA.prod, 2026-07-17)
Clean `origin/test/mcdonges.latest` @ **93de8ac** worktree; `OOSH_DIR`=worktree so the FIXED otmux is under test; live tree untouched, worktree removed. Independently reproduces the expert's smoke from a DIFFERENT caller pane (cross-validation).
- **(1) static** — ZERO active `$TMUX_PANE` self-ID CODE in otmux/hiveMind/claudeCode; all 7 remaining hits are ban-comments (incl. the trailing comment on the line that calls `private.otmux.pane.resolve`); hiveMind + claudeCode have zero occurrences. → guard #1.
- **(3) definedness** — `otmux.pane.self()` DEFINED in source AND `otmux pane.self` → `%4` (non-empty, real pane) — closes the live "called-but-undefined→empty" break I filed in the 42-context test (AC4). → guard #3.
- **(2) poisoned `TMUX_PANE=%999`** — `otmux pane.self`=`%4`, `otmux current`=`ooshTeam:0.4`, `otmux pane.get.target`=`ooshTeam:0.4`: NONE leaked `%999`, ALL returned the REAL caller pane, and poisoned==clean (resolver is env-immune) — the move/rewind lie is fixed. → guard #2.
- **Cross-validation**: resolver returned MY actual pane `ooshTeam:0.4`/`%4` (oosh-tester), where the expert's smoke returned `ooshTeam:0.3` (expert) — each caller resolves to its OWN real pane via the PID-walk. No gap surfaced.
- Proof: `session/tasks/T-NO-TMUXPANE-EXT.proof.txt`; harness `scratchpad/t-no-tmuxpane-ext.sh` (ready to land as `test/test.no-tmuxpane` extended). → PO gate → Tron → ff-deploy live.
