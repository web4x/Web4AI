...
claudeCode.agent.start() # <?workdir:.> <?--model:sonnet> # start Claude Code with optional model
{
  ...
  if [ -n "$TMUX" ]; then
    tmux send-keys "cd '$workdir' && $cmd" Enter
...
Update `hiveMind agent.bootstrap` to accept `--model` parameter:
```bash
./hiveMind agent.bootstrap oosh-expert --model opus
```
...
- [ ] `claudeCode agent.start /path --model opus` starts with opus model
