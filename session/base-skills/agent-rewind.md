...
> **⚠️ CORRECTED (ARON 2026-08-12, hit it live driving the expert): `pane.size.set` does NOT always solve this — it CLAMPS in a WINDOW-PINNED-SMALL window.** tmux sizes a window to the **smallest attached client**, so an 8-pane team pinned at 83x19 gives NO pane more than 19 rows — `pane.size.set 90 46` clamped to 19, the picker rendered header-only, and a raw `resize-pane -Z` zoom would ALSO cap at 19 (it fills the *pinned window*, which is the whole problem). So "ENLARGE OOSH-clean" is **necessary-but-INSUFFICIENT under a pinning client**, and this **is NOT a retired workaround** — it is an **OPEN GAP → owed sprint: `otmux pane.present <pane> [minRows:45]` + `pane.restore`**, which must lift the WINDOW past the pinning client (window-size manual + resize-window, or manage the client, or a tall driver-window), **RETURN the achieved rows**, and the driver **MUST verify ≥ minRows before opening the picker** (a silent under-reach hands a header-only picker = the blind-navigation hazard = vacuous verification). **Until `pane.present` ships: a rewind in a pinned many-pane window is BLOCKED** — do NOT navigate a header-only picker blind (overshoot = uncontrolled restore) and do NOT raw-zoom (it caps at the pinned height anyway); abort clean, leave the agent committed-safe, and un-pin the window (client management, TRON's ok) or bound the agent's work instead. **Severity: without `pane.present`, any multi-pane team has agents that CANNOT be cleanly rewound = a recoverability SPOF.**
