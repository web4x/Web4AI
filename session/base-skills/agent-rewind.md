...
- **Truth over MULTIPLE sources** — an agent CANNOT read its own context (the 42 principle). Predict/measure it externally: pane footer (`Context low`/`N% remaining`), `claudeCode context.read`, transcript token count, message-count trend. Cross-check ≥2 (one instrument lies). **`claudeCode context.read` can only prove SAFE-LOW — it can NEVER establish "safe enough."** It UNDER-READS the authoritative `/context`, and the error GROWS at high usage (measured 2026-08-08: SM +8, req +2.5, **planner +18** — a genuine near-wall read as ~68). Graduated rule (po, fleet-wide): read **<40 ⇒ dispatch freely** · **40–55 ⇒ bounded work only** · **>55 ⇒ STOP — get an authoritative `/context` at genuine idle (peer panel-capture) BEFORE any dispatch, or just route the rewind.** **NEVER size UNBOUNDED work (grep-first / untangles / migrations) off a read at ANY level.** "I have no fresh number" is a REAL RISK REPORT, not caution — measure before assigning. context.read reports **%FREE** (used = 100 − read).
