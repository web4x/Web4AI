...
7. **Short pointers, never long prose on the wire.** Long/wrapping messages wrap-stall unsubmitted and lose spaces. Write detail to a committed file; send a short `Read <file>` pointer. [[sprint-comms-protocol]] **Corollary — NEAR THE WALL, send SHORT only:** a message costs the recipient tokens to receive. Sending a LONG message to a near-saturated agent (≤~5% free) can TIP IT to 0% (measured on robbin-po: a long reminder pushed it 2%→0%). To a near-wall agent send a one-line pointer, or free room (Phase-1 rewind) first.
