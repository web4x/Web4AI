...
**L-act-on-evidence-claim-only-on-observation (2026-08-24, PO, general rule + my own over-claim):** we ACT on the best available evidence; we CLAIM only on OBSERVATION. I recorded 'board-liveness == #3, no separate subscription' as SETTLED - but it rested on STATIC readings (architect code-read + planner rb-object-item:73) + an ABSENCE of observed staleness (not falsification). Nobody had OBSERVED a regular badge at runtime. A static code-reading is evidence to ACT on, NOT an observation to CLAIM. The planner's earlier equally-careful container-reading turned out WRONG => replacing inference-A with inference-B and recording it settled is the SAME error one layer along, and in a durable unit it forms FALSE CANON. Fix: qualify to RUNTIME-UNVERIFIED, name the FALSIFIER (tester observes the badge @390), flip only on observation. This is measure-not-relay applied to my OWN recording - the discipline binds the recorder hardest. [[measure-the-property-not-a-proxy-both-directions]] [[static-reading-is-not-observation]]

**L-artifact-drifts-from-reality-is-one-family (2026-08-24, PO, 3rd instance):** a STALE BOOT misdescribing the sprint (R40.55), a STALE GATE GREP for a retired code shape (r4017 false-red), and a STALE COMMENT/AC misdescribing the code (CurrentSprint.ts:296 code=not-Done broader than comment {P/IP/QA}) are ONE family: an artifact drifting from the reality it describes. A correct reader reasons FROM the drifted artifact and is WRONG about the system (how the planners container-reading went astray). Three times = the artifact set needs a MAINTENANCE property. When a comment/AC states a constraint the code does not enforce: FLAG it (intent-not-held must be VISIBLE not silent), do NOT silently align either side - the design-owner rules which is correct (narrow the code or widen the artifact). A gate written to the drifted artifact passes/fails against an intent the code does not implement. [[measure-the-property-not-a-proxy-both-directions]] [[act-on-evidence-claim-only-on-observation]]

**L-assert-the-negative-a-happy-set-gate-is-vacuous (2026-08-24, PO, sharpest of the round):** when the bug is that CODE ACCEPTS something the SPEC does NOT mention (CurrentSprint.ts:296 !==Done denylist leaks Superseded/Cancelled), a gate written to the happy-set {P/IP/QA} PASSES the bug - it only tests statuses BOTH the correct-AC and the buggy-code accept. The bug lives ENTIRELY in what the code accepts that the AC omits. => the gate MUST assert the NEGATIVE: enumerate the EXCLUDED set {Done|Superseded|Cancelled} and test that a Cancelled/Superseded task is NOT designatable (RED on todays code). A happy-path-only gate is vacuous exactly the way the weaker-property misses were. ★ AND: routing a divergence to the owner (not silently aligning) caught a REAL LATENT DEFECT - widening the ACs to match the code would have BLESSED a Cancelled task as current = answer-fitting that sanctions a bug. narrow-the-code vs widen-the-spec are DIFFERENT PRODUCT DECISIONS; the owner rules, never a silent tidy-up. [[flag-dont-silently-align-either-side]] [[measure-the-property-not-a-proxy-both-directions]]