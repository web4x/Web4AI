...
**BOOT FIRST**: `otmux pane.history <self>` + `ls scrum.pmo/sprints*` (per session/base-skills/agent-rewind.md). **Identity**: oosh-po@WODA.prod, ooshTeam:0.0, uuid 29a1e1d1.
...
**PRINCIPLES banked (reading-list/learnings)**: T2Q=minimize tokens/task-to-QA=reduce noise · scalability>primitive (QA=repeatable captured proof, not eyeball) · dual-link=`[GitHub](url) | [relative/path]` same artifact · send-canon (`send.tui` in modals) · PO doesn't run tests (tester) / doesn't poll (SM) · measure-source-not-copy.
