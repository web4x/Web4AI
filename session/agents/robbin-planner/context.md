...
- **CURRENT TASK = BUG8 navigation fix** (PO CORRECTED: the 4 DeFED.net pictures are NAVIGATION BUGS to FIX, not a rendering to adopt). setChain ok=true → live singleton flipped to BUG8; WIP=req hop 0. Family order: BUG8(12cf7bb5)→BUG9(1b216edc)→BUG10(da4a27bc)→BUG11(871c5cf9). Goal: clicking collections/files/links in the room tree NAVIGATES correctly.
  ...
  - ▶ NEXT GATE = DEPLOY v0.6.28 + tester REPRODUCE on live DeFED.net (live room is STALE-DEPLOYED <v0.6.28 → still shows bug). Verify-on-real-data = deploy+tester.
  - ⚠ NAME-MATCH FLAG (seal, not pin): impl 36934fe3 lives INSIDE method renderDetailForRef (rb-detail-drawer.ts:84, marker R20.10 dbddf408), NOT openForRef. No openForRef method in this file. Pin uses 0a902bff(openForRef) per architect but HONEST seal needs Method=renderDetailForRef node + RED test + det-3x. DO NOT credit on 0a902bff.
