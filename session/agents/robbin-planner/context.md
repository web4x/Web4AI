...
- ⚠ **BUG9 altId COLLISION (source-verify catch):** 1b216edc (leaf-renders-detail) + 6da84135 (forward-keys) BOTH active = BUG9. Flagged to PO (renumber 1b216edc→BUG14? PO's call). Not fixed unilaterally.
- ✓ **WIP NOW = BUG13** (.url click opens drawer, req f420e98a, task 10545051 in-progress). Pin set via setChain (focus 10545051 = ok=FALSE — focus needs a DERIVABLE chain req→uc, BUG13 has no uc yet; architect must mint uc for focus auto-follow). Tester writes RED now. LESSON: `focus` needs task with built chain (≥req→uc), not just coveredRequirements.
