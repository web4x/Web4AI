...
- ⏳ **PIN STALE (BUG8) — awaiting unit-sourced-trace task UUID from PO.** WIP moved to Phase 2 (unit-sourced /api/trace migration) but couldn't ID the exact task unit (T163/T166/R17.18 are chain-less old candidates; req pane stale showing b7894ac3 as R20.15 — I renumbered→R20.17). Asked PO for uuid; won't pin a guess (wrong pin defeats the realtime ask). ★ NEW TOOL: `planner-drive.ts focus <task-uuid>` AUTO-DERIVES chain from a focused task (PREFERRED over manual setChain — skill-expert's auto-follow wiring). Use `focus <uuid>` once PO gives it. RULE (PO, recurring): WIP=1 changes → setChain/focus IMMEDIATELY, pin always = current work. Durable fix = auto-follow so pin never depends on manual re-pin.
