...
- **CURRENT TASK = BUG8 navigation fix** (PO CORRECTED: the 4 DeFED.net pictures are NAVIGATION BUGS to FIX, not a rendering to adopt). setChain ok=true → live singleton flipped to BUG8; WIP=req hop 0. Family order: BUG8(12cf7bb5)→BUG9(1b216edc)→BUG10(da4a27bc)→BUG11(871c5cf9). Goal: clicking collections/files/links in the room tree NAVIGATES correctly.
  ...
  - ✓ LIVE = v0.6.28 (re-confirmed /api/health uptime 224, 17 rooms — NOT stale). ⛔ **BUG8 STILL RED on live DeFED.net** (tester): v0.6.28 uuid fix NECESSARY but INCOMPLETE — collection click opens drawer in CHAT mode, children=0 (NOT collection-children). Deeper layer: renderDetailForRef shows chat not collection-children → collection branch likely not entered / mode forced to chat. ▶ architect re-diagnosing deeper root-cause. HOLD champagne 26 — BUG8 NOT sealed. Verify-on-real-data caught the incomplete fix (unit-green would've missed). BUG8 stays current task till GREEN on DeFED.net.
