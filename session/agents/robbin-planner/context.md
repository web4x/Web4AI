...
**P2-follow2 ✓ R20.5 (7734f4e1, LIVE/user-facing) content-accuracy (ef9d5ba56):** clause (A) chain depth was STALE (req→uc→class→METHOD truncated) → FULL DEPTH (req→uc→class→method→impl→test→gate) per R20.30 chain-descent; (B) all-children-union + (C) dedup-traceability accurate, left. → STANDBY (R20.30 gate = active front).
