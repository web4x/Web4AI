...
- ▶ **CURRENT WIP = R20.20/21 describe/TestCase (REOPENED, NOT done).** Tron IMG_4045 'No chain/no links' — gate-proven (test PASSES) ≠ chain RENDERS. SOURCE-VERIFIED 2 gaps: (1) Test.testCases[] EMPTY — 1016 TestCase units have testUuid→Test but reverse Test.testCases[]→TestCase not populated, so forward-key Test:['testCases'] renders nothing; (2) Method parseFromSource (499b453f) impls[]/tests[] empty → chain dead-ends. Reverted QA-Review→in-progress. DISPATCHED expert: reverse-index Test.testCases[] + wire Method impl/test (self-mark hops). Commit 0b6e11886. NO IDLE.
