...
- ✓ **WIP NOW = task 54519bc4-2704-4484-a83f-5b88019d62c3** (R20.20 TestCase 1e3f9799 + R20.21 Gate 102ab818 = describe-tests + gates as 1st-class scenario units; chain …→Test→TestCase→Gate; architect designing the new types). Pin via setChain (focus ok=FALSE — task ucs=[] no uc yet). Re-run `focus 54519bc4-2704...` once architect mints uc → auto-follow.
- R20.19 = QA-Review (don't block other work). R20.20/21 + 54519bc4 the active WIP. LESSON: `focus` needs derivable chain (≥req→uc); ok=false on req-only/no-uc → setChain fallback.
