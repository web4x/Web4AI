...
## 87. MEASURE THE KIND before standing up; existence != connection; right-granularity; checkpoint-with-margin (realtime-MVC window, 2026-08-17)
Standing up the S37 realtime-MVC tasks off a PO/req handoff, four disciplines held and are worth banking together:
- **(a) MEASURE THE KIND, don't obey the frame.** The PO + req both called them "5 realtime-MVC TASK units — stand them up." On disk they were **UseCases**, not tasks (only ae01f065/T37.20 was a Task). Standing-up = MINTING task units (planner-mints-tasks) across the covering reqs, not adopting 5 existing tasks. Measuring the ior of each uuid re-derived the honest shape (2 mints + 1 link, later 3 mints + 1 fold). The PO named it their own KIND-confusion (it had also quoted a Task uuid as an Impl uuid the same day) — the catch is the same class as the UC-vs-Impl / req-vs-task confusions that cross-wire chains. NEVER mint off a uuid without confirming its ior first.
- **(b) EXISTENCE != CONNECTION (the F8 lesson).** Task 37.24 EXISTED (minted, valid on disk) but sprint b86b53cc.tasks[] and R37.11.tasks[] did NOT reference it (grep=0) → it was invisible from the sprint side, would never render under Sprint 37 (the R40.18 lesson: a task the sprint can't see is the complaint Tron opened with). A minted unit needs the BIDIRECTIONAL reverse-wire to be real. ★ And before wiring: I RE-MEASURED and found it had ALREADY been reverse-wired (dde0845c9) in the ~1 min since the PO measured 0/0 — so I did NOT double-wire (a duplicate tasks[] entry). "Check first whether it's already done" beats "obey the instruction to wire it" on a churning tree. Then the board was still STALE (units wired but not regenerated) → render is a SEPARATE step from wire.
- **(c) RIGHT-GRANULARITY = anti-over-decomposition.** R37.12 had 2 new UCs (subscribe-on-render + one-bus) = ONE coherent view-side deliverable → ONE task, not per-slice (over-decomposition we'd collapsed before in R30.11). But R40.4 had TWO distinct DoDs (formatter that closes-when-gated vs migration that self-drains-to-0) → TWO tasks. And a UC that's a 2nd facet of an existing task's req (dnd e3fcf5b3 = R37.20's 2nd UC) FOLDS into that task's useCases[], no new task. The unit of a task is a DoD, not a UC-count — measure the DoDs, coordinate the fold-vs-split with req (they own the reqs/chain), scenario-first.
- **(d) CHECKPOINT-WITH-MARGIN > heroic-finish (twice this window, PO named it).** At a PO-bounded "STOP at ~75%": I stopped at 62% before a multi-task mint job (reported the plan, held for confirm) and again at 72% after mint 1/3 rather than push into mint 2/3 under context pressure on Tron-watched work. A clean stop with rewind-margin beats finishing heroically at the wall — the PO walls agents by dispatching big jobs without measuring first, so a bounded stop protects both sides. Same spine as [[prevent-cliff-proactive-rewind-90]] but at the per-task grain.

## 88. Turn a one-off sweep into a REPEATABLE INVARIANT GATE; self-drain PROVEN not asserted; never interrupt a driver mid-picker (2026-08-17, PO-named)
Three disciplines the PO explicitly banked back to me this window:
- **(a) A VERIFICATION SWEEP BECOMES AN INVARIANT when you productionize it — the difference between finding a gap and making it impossible to accumulate.** Asked to verify TWO mints' reverse-wiring, I swept ALL 19 tasks / 23 reqs (name-the-FAMILY-not-the-instance) and found a real gap nobody was looking for (R37.4.tasks[] missing subtask T37.4.1). Then I turned the scratchpad script into `scripts/reverse-wire-audit.mjs` — OWNED like checklist-chain-audit.mjs: FAIL=dangling-ref(strict) / WARN=reverse-asymmetry(report-only + COUNT emitted, self-draining), a 4/4 stub-must-fail BITE proving non-vacuity, and the PO's cross-sprint ruling baked in AS a bite case (D: cross-sprint but SYMMETRIC = NOT a gap — the detector checks wiring symmetry only, never sprint-membership). "existence != connection" now cannot re-accumulate silently. Extends #87(b).
- **(b) SELF-DRAIN PROVEN END-TO-END, not asserted — this is WHY we ship gates report-only FIRST.** req closed the R37.4 gap (7953156f4); I RE-VERIFIED on disk (not on its report — [[verify-with-independent-method]]) and the gate went WARN 1→0 BY ITSELF. The report-only-then-strict / count-emit / auto-strict-at-0 pattern DEMONSTRATED rather than claimed. Cite this whenever anyone questions why a gate ships report-only before strict: the count draining to 0 IS the readiness signal to flip strict.
- **(c) NEVER INTERRUPT A DRIVER MID-PICKER — peer-measure only at POST-TURN idle (idle != quiet).** PO's bounded job = peer-measure the trainer's /context (an agent can't self-invoke it). The trainer was mid-drive ("executing expert /rewind", 21m43s thinking, driving another pane's picker). Injecting /context there = an open modal nobody can finish + a garbled read (a finishing-turn re-render dismisses modals). "Idle" = POST-TURN, not a quiet-looking composer. DEFERRED, reported it, stayed ready. When I do measure: read the TOTAL "Nk/1m Npct" line ONLY — never a sub-metric (the trainer once reported "File reads 63.3k=6pct" as its total; the PO dispatched 3 mints on that ghost when the real number was 72%); grow the pane if the render wraps (restore after). Also: **ci:gates registration is the EXPERT's lane + the PO's to ROUTE — not mine to chase; my lane stops at package.json.** [[context-read-1m-denominator-stale]].
- **(d) PROVE a composer is a GHOST vs REAL with a NON-EFFECTIVE C-u (PO banked, they hit this wrong today).** Before injecting /context over a peer pane whose composer shows text: send C-u. REAL typed text prints the cut-notice + EMPTIES the line; a GHOST suggested-prompt gives NO feedback (unchanged). A non-effective C-u ⇒ it's a ghost ⇒ the composer is effectively empty ⇒ /context lands clean. Do NOT mis-diagnose ghost text as a wedged/occupied composer (and do NOT refuse the measure over a ghost). Extends [[ghost-suggested-prompt-not-real-text]]. Concrete: trainer composer "measure me with the planner now" survived C-u = ghost.
- **(e) VERTICAL SCROLL-OFF is NOT WRAP — diagnose by WIDTH, fix the right axis (PO banked, they mis-diagnosed as wrap + captured-deeper instead of growing).** A missing TOTAL "Nk/1m Npct" line has TWO causes: WRAP (a NARROW pane folds the render → grow WIDTH) vs VERTICAL SCROLL-OFF (a SHORT pane pushes the top above the viewport → grow HEIGHT). Diagnose by pane WIDTH: ≥~120 cols ⇒ NOT wrap ⇒ it scrolled off vertically ⇒ grow HEIGHT via pane.size.set, read, RESTORE. Capturing-deeper does NOT fix a vertical scroll-off (the top is past the scrollback the capture reaches); growing height + re-render does. Concrete: trainer @ 194x21, total off-top; grew to 194x42 → 688.2k/1m 69% visible → restored to ~21. Both (d)+(e) handed to the trainer for agent-rewind.md canon (it owns canon, can't read /root/.claude) → LANDED as agent-rewind.md measurement-family **#8** (ghost-via-C-u) + **#9** (wrap-vs-vertical-scroll-off), trainer-committed, cited as my relay. The full CMM4 loop: a measurement trap I navigated became shared canon — and the trainer confirmed the canon then worked on its own measure.

## 89. Board-honesty is a DIRECTION, not a symptom; a regen can UNDERSTATE or OVERSTATE; measure the ONE source; refuse to invent (2026-08-17, PO-named)
A dense window (Tron caught lapsed version-tags + inconsistent sprint-numbering + a live board-drift). Four disciplines PO explicitly banked back:
- **(a) BOARD-DRIFT has a DIRECTION and a CAUSE — diagnose, don't hand-patch.** T37.24 showed 1-of-4 while its unit was 2-of-4. I measured 3 causes: (b) generator-can't-render-substeps → RULED OUT (--check flagged the md = generated≠disk); (c) gate-blind → RULED OUT (check:sprint-md CAUGHT it); (a) regen-never-FIRED → CONFIRMED (req's tick commit touched ONLY the unit, no MD regen). The accepted split (hook regens scoreboard/rows; per-sprint task-MD "gated by check:sprint-md" separately) leaves exactly this hole: the gate catches drift only at CI, the regen never covers sub-step ticks at commit-time. Fix = regen from the unit (generator, NEVER hand-edit — a hand-edit HIDES the defect on a generated file). The hook-fix is the expert's; my regen makes the view honest + surfaces the defect.
- **(b) A REGEN CAN OVERSTATE, not only understate — a mass status-move must NOT ride silently.** Sprint-19 regen would move 25/27 rows Done/QA→In-Progress (the OPPOSITE of the understate complaint). I HELD it (restored to HEAD, 0 committed) and flagged PO — because downgrading 25 rows on an old sprint needed a ruling. DECISIVE measure: NONE of the 25 had approvedBy → per R40.10 Done is Tron's act → they were never Tron-Dones = legacy checklist-derived FALSE-GREENS; the R-C9 done-provenance revert had corrected the UNITS but the VIEWS were never regenned. PO sanctioned regen-down (deletes a lie, 0 real Tron-Dones lost). Understating (hiding shipped work) and overstating (false-green) are the SAME lie in opposite directions; flag the direction, cite approvedBy, hold a mass move for sanction. [[done-carries-provenance-not-bare]] + [[measure-a-stable-state-not-a-moving-target]].
- **(c) MEASURED beats ESTIMATED, and there is ONE source — don't let two counts diverge.** PO's numbers were corrected TWICE by my measurement (~100→157 tags; "standard starts v0.7.92"→measured v0.6.0, since v0.7.92 was merely the 2nd lapse). The 357-vs-514 "divergence" was the SAME set at different scopes (357 pre-standard + 157 in-window = 514). FIX: expose the enumeration as THE single source (scripts/release-tag-audit.mjs, --json) that BOTH release-ops AND the tester's gate CONSUME — two independent version/tag counts is the two-source disease. My estimates are inputs; the measurement is the record.
- **(d) REFUSE TO INVENT for a version/row that never shipped — declare it instead.** Tagged the EXACT ship commit (introduce-commit rule, not HEAD — a wrong tag is worse than a missing one); v0.8.40 / v0.7.30 never appear in package.json = never shipped → NOT tagged, DECLARED as gaps. Pre-standard (~300 pre-v0.6.0) intentionally untagged = DECLARED, not defaulted (an undeclared gap is omission-by-default). The tag-on-deploy MECHANISM (expert) + the served==committed==tagged GATE (tester) + this record = the three parts that stop a standard from decaying into a habit (it had lapsed TWICE, and backfill#1 was itself incomplete = 57 in-era gaps). Same pattern: sprint-number inventory (38 sprints, all have the number attribute; 24 embed-in-name S10-33 render numbered, 14 bare S1-9+S34-40 don't) = the acceptance DATA for a renderer fix — measure the split precisely, HOLD further stripping until the renderer composes number-from-attribute (else each strip makes it MORE inconsistent).
- **(e) LOOK BEFORE YOU DELETE — an orphaned-LOOKING artifact is usually MIS-NAMED, not abandoned (PO named it twice in one day).** A generated-header MD with no expected slot ("extra" in --check) reads like a deletion candidate, but MEASURE first: does it carry a task/unit ref, does that unit EXIST, is it in the sprint's tasks[], is there a CURRENT MD under the unit's current slug? sprint-18's svg-fullscreen-iframe-native-zoom.md was a STALE-SLUG DUPLICATE of a RENAMED task (unit bef36fd2 alive, current MD task-r18.34-… 612 lines vs the stale 32) = safe-delete, 0 loss. The SAME day, 4 "delivered-but-unchained features that read as no-deliverable" were also rename-leftovers, not lost work. A TRULY missing unit (MD references a uuid with no unit on disk) is the opposite = do NOT delete (re-mint or preserve). The delete is only safe once you've PROVEN the content survives elsewhere (unit + current view); then git rm path-limited (never -A), record the WHY, note it's git-recoverable. PO now independently VERIFIES delete-evidence (612-vs-32 lines, same unit) rather than confirming on the report — measure-both-sides is the standard, not distrust.
