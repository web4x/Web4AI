...
## 13. New sprints MUST be navigable (discoverability) — PO directive 2026-05-26
Whenever standing up a new sprint, add it to BOTH (a) the README.md "Individual sprints" list and (b) `scrum.pmo/sprints/sprints.overview.md` IN THE SAME COMMIT. Reason: Tron navigates via README; the list had stopped at S3 so S4-S14 were invisible despite existing. The durable index `sprints.overview.md` (number, name, status, task count, planning link) is the canonical sprint catalog. Also: when the PO says "push" (or repo is ahead), `git push` so origin/GitHub is current — don't leave commits local-only.
