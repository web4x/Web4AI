...
- **⚠ OPEN**: (a) tester must update `test.dispatch-submit` GATE-SRC (structural, locked old `-ne 0` gate → now `-eq 3||-eq 1`). (b) **NOT declared fixed until TRON confirms LIVE.** If dups persist: candidate 2 (SM/drain process running STALE sourced hiveMind — needs re-exec to pick up d4e3ae0) or candidate 3 (agent re-sends on rc2/"uncertain" from the MAIN inform path). (c) Policy note flagged to PO/architect: rc2→dequeue changes OTR-1's "dequeue ONLY rc0" invariant.
