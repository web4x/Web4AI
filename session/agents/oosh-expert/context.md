...
Tron/PO: send.verified's poke loop sprays blank Enters (demo caught 2× poke on a shell). Architect redesigning CLEAN: **stage-once → Escape-dismiss-if-claude+idle → SINGLE Enter → verify → honest rc {0/2}, NO poke/retry loop; shell = single-Enter, NO Escape, NO poke.** **DO NOT START until the architect's design lands** (Tron: no oscillation). I'm on HOLD. NB: my task-s2-j `otmux.rewind.drive` is ALREADY poke-free (raw send-keys + single Enter for select, does NOT use the send.verified poke loop) → the redesign won't conflict with it. When design lands: implement in otmux send.verified/send.smart.
