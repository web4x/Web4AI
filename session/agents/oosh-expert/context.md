...
---
## ⏸ PENDING (2026-07-03, HOLD for architect design) — REMOVE POKE from send.verified
Tron/PO (reinforced): the poke LOOP sprays blank Enters (demo caught 2× poke on a shell). **KILL IT ENTIRELY — not cap it.** Target shape (the redesign should DELETE most of send.verified): **CLAUDE** = ONE Escape (dismiss autocomplete, idle-only) → type text → ONE Enter → (optional: ONE capture → honest rc{0/2}) → done. **SHELL** = type + ONE Enter, NO Escape. **NO for-loop, NO poke, NO maxpokes, NO retry, NO 'poke' logs, NEVER re-Enter.** **DO NOT START until the architect's (imminent, trivial) design lands** (Tron: no oscillation). I'm on HOLD. NB: my task-s2-j `otmux.rewind.drive` is ALREADY poke-free (raw send-keys + single Enter for select, does NOT use the send.verified poke loop) → the redesign won't conflict with it. When design lands: implement in otmux send.verified/send.smart.
