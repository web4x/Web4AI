...
## NEW: init/oosh is `#!/usr/bin/env sh` and IS dash-safe (self-re-execs to bash) (2026-07-02, #13 D13.A)
Unlike the rest of OOSH, `init/oosh` is deliberately POSIX sh — it's the fresh-host bootstrap. Measured on WODA.test: `dash -n` rc=0; runs under dash in BOTH the file-arg form (`sh init/oosh`, `$0`=path) AND the README curl-pipe form (`sh -c "$(cat init/oosh)"`, `$0`=dash) through to framework handoff; `OOSH_DIR` resolves correctly in both (pipe-form `$0`=dash handled by a pre-clone fallback + a re-exec-to-bash ~line 287). No bashism failure reproducible. Sandbox-HOME runs surface `mkdir ''`/`state-not-found`/missing-`.ssh` — those are CONFIG-gap artifacts of a throwaway HOME, NOT dash-vs-bash issues. When measuring an installer you can't fully run, name the sandbox artifacts explicitly so they aren't mistaken for the bug.

## NEW: install-body POSIX forms (2026-07-02, #13 D13.2, ef34ed0)
`read -r` is POSIX (portable); `read -p`/`-n` are bash-only → `printf '%s' "prompt"; read -r REPLY`. `&>` is bash → `>/dev/null 2>&1`. `[[ x =~ re ]]` → `case`. `[[ ":$PATH:" != *":$d:"* ]]` → `case ":$PATH:" in *":$d:"*) …`. `source` → `.`. `config.validate` (Rule A, config:473) allows BOTH `source *.env` and `. *.env` in a pure-state env file, so writing the `.` form keeps user.env valid + portable. `dash -n` is INSUFFICIENT to verify — `[[` and `read -p` PARSE ok under dash then fail at RUNTIME; verify constructs by actually running them under dash.
...
## NEW: direct-root install skips .bashrc wiring — wire it IN-CHAIN as a state (2026-06-28, bee01a1)

The installer wires a user's `~/.bashrc` (the OOSH bootstrap) via `user oosh.install $SUDO_USER` ONLY when `SUDO_USER`≠root. A DIRECT-root install (ssh as root) has empty SUDO_USER → root's bashrc never wired → `env -i bash` isn't an OOSH shell (empty OOSH_MODE/DIR, no oo) even though the state machine completed. Fix: wire root's bashrc as a deterministic SETUP_SERVER STATE step (in `private.check.root.installation.done`), installing `templates/user/bashrcTemplate` idempotently (skip if already wired, back up original). **Lesson**: clean-boot needs the bashrc bootstrap, which is SEPARATE from state-machine completion + config/symlink setup. Make environment wiring in-chain/deterministic, not a post-install side-step that a code path can skip.

## NEW: pkill -f self-match — tag background loops with a distinct signature (2026-06-28, BUG6 regression 44c9043)

`pkill -f "pane.lock.*<target>"` (meant to sweep orphan enforcer loops) ALSO matched the FOREGROUND `otmux pane.lock <target>` process's own argv → since pane.lock auto-unlocks first, it SIGTERM'd ITSELF (rc=143, title never set). Fix: launch the background loop with a DISTINCT signature in its argv (`setsid bash -c '…' __paneLockEnforcer <target> <title> <pidFile>`) and pkill ONLY that signature — never the generic script name. **Lesson**: `pkill -f` matches the WHOLE command line; a foreground invocation and its background child often share the script name. Always tag spawned loops with a unique marker token and target THAT. Escape regex-significant chars in the target (`0.4` must not match `0.41`).

## NEW: hermetic install — prefer in-repo assets over external wget (2026-06-28, ffb38c9)

A fresh server install FATAL-aborted at a `wget -r` of `developking.ssh` templates from `test.wo-da.de` (recursive 404 on a sub-asset → non-zero → `return`). The asset already SHIPS IN THE REPO at `templates/user/developking.ssh`. Fix: copy from `$OOSH_DIR/templates/...` — no external dependency, the repo is the source of truth. Also: `if [ ! -d /root/.ssh ]` guarded BOTH keygen AND config-write together, so a box with injected keys never got `~/.ssh/config` → downstream `cat` failed; split into create-if-missing per artifact (self-care). **Lesson**: external download in an install path = a 404/network SPOF; if the asset is versioned, ship it in-repo and copy. Guard each created artifact independently (create-if-missing), never bundle unrelated setup behind one existence check.
...
## NEW: submission-verify by INPUT-LINE REGION, not text-presence (2026-07-02, OTR-1 96ccff2)
`send.verified` greped for the text ANYWHERE in the capture → matched text echoed in the transcript SAME as text staged-but-unsubmitted on the `❯` input line → false "delivered" (BUG10, unsent dispatches all session). Fix: verify by REGION — SUBMITTED iff the staged text LEFT the input line (empty ❯ / `esc to interrupt` processing); STAGED iff still after the ❯/prompt marker; BLOCKED iff overlay/permission marker. Object.verb primitives stage/submit/poke/verify; submit/poke text-free (idempotent — N pokes can't dup). HONEST rc{0 submitted/2 staged-unverified/3 blocked/1 error}, never "return 0 / verify manually". **Rule**: "keys sent" ≠ "submitted"; a controller send is DONE only when submission is verified. LONG payloads WRAP → 1st Enter=newline→never submits→honest rc2 (not papered over); pointer-only payloads (delegate) avoid wrap → submit by construction (ARON#3). GATE queue-dequeue on rc0 or you silently drop unsent messages (robbin-po unreachable).
...
## NEW: honest env -i clean boot — self-derive HOME from passwd, source siblings self-relative (2026-09-08, e3222de)
Tron principle: OOSH must boot from a LITERALLY empty `env -i` (no inherited state = constructor/no-state-interference). Rigging it by feeding HOME/PATH is FORBIDDEN — no human does that; if it needs a var, THAT is the bug. Two real failures measured (feed NOTHING: `env -i /bin/bash -c '"$OOSH_DIR/oo" mode.list'`):
1. **`source this` needs PATH** — under env -i, bash injects a DEFAULT compiled-in PATH (`/usr/gnu/bin:/usr/local/bin:/bin:/usr/bin:.`) so SYSTEM tools (`id`,`dscl`) resolve, but OOSH_DIR is NOT on it → `source this` (a non-system sibling) fails rc=127 "this: No such file or directory". Fix at the ENTRY script only: `source "$(dirname "${BASH_SOURCE[0]}")/this" 2>/dev/null || source this`. `this` then does `PATH="$OOSH_DIR:$PATH"` so every downstream `source X`/sibling call works — no need to patch all N scripts' `source this`.
2. **empty HOME → `CONFIG_PATH=/config`** (`: ${CONFIG_PATH:=$HOME/config}`). Self-derive HOME from OS IDENTITY, pure bash, OS-independent: `eval echo ~"$(id -un)"`. Bash tilde-user expansion consults the passwd DB (getpwnam) on macOS AND Linux even when HOME is unset — NO dscl/getent per-OS branch needed. Guard `[ -z "$HOME" ]` (normal boot pays one test); reject failed expansion `case "$d" in ''|'~'*) d="" ;; esac`. Place at this top-level after the OOSH_DIR derivation block so HOME is ready before CONFIG_PATH + the oosh.dir.invariant ($HOME/oosh).
**Lesson**: "boots clean / no state interference" is tested by feeding env -i NOTHING; measure the ACTUAL in-process HOME/OOSH_DIR/CONFIG_PATH (child exports don't propagate to the outer shell — echo them from inside). OOSH_DIR self-derives fine (BASH_SOURCE `cd;pwd`, logical, keeps symlink); HOME was the gap. Honest CMM4 gate = T-ENV-I in test.this. Ties [[env-i-honest-boot]], [[expert-dogfoods-oosh]].
