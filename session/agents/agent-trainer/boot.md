...
- NO GIT REBASE. EVER.
- Peer Compact Protocol: trigger save → wait → /compact → verify. Never raw /compact.
- Boot.md edits: one by one, read before add (Tron directive).
- No bulk Python writes — individual Edit tool calls only (Tron directive).
- CHECK = behavioral (CMM4), not just file grep (CMM2).
