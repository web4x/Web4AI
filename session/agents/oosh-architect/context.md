...
- **task-s2-i shell.reap** (4d670b5): reaper companion to h (h SHOWS count, i REAPS). Safe-reap CORRECT-BY-CONSTRUCTION — reap ONLY provably-safe (ORPHANED=owning pane no live claude/reparented-to-init; IDLE-STALE=no running child + age>TTL), DEFAULT=KEEP on doubt; ACTIVE-WORK + pane-foreground NEVER (guards proven pane.lock otmux:3119 SIGTERM-live-foreground + BUG6 loose-pkill → kill BY PID, SIGTERM→re-classify→SIGKILL). Reuses h tty→pane_pid subtree (DRY) + claudeCode.process.running. Triggers: rewind (hiveMind:530 = closes persist-thru-rewind = OTR-2 fd-leak cure) + idle-sweep. shell.reap.dry = safety valve.
- **★ SPRINT-2 ARCHITECT DESIGN COMPLETE (2026-07-02, PO-confirmed):** entire remaining sprint (C-family c.0/c.2/c.3 + g.1/g.2/g.3 + h + i) designed as PROJECTIONS/REUSES of the ONE c.0 live-reader. c.0=reader; parity/C.2/C.3/dashboard=projections; h/i=subtree reuses. Coherence-verified (d25bc18). All ahead of expert impl (blocked on Tron /rewind). **Next on rewind: verify expert impl against these specs; c.0 built FIRST (everything depends on it).**
- **task-s2-g.5 send test matrix** (T-SEND-MATRIX, 11 groups/50 cells, cc8dbdf/2a6e3fa): EXHAUSTIVE permanent send regression-guard, superset of DISPATCH-SUBMIT(5)+SEND-SESSION(3). Tron's 5 essentials incl E5 prefix-once (BUG9) + GROUP K single-key (send.raw≠message).
- **send-saga tail** (all against send.verified 2fdce8e = the current send: stage-once + verify-COMMIT + Enter-only poke, NEVER resend): **g.6** coherence review (2fd1691) — found 1 HIGH (last-24 probe checks only ❯ row → false-committed on WRAP), 1 MED (local-tmux-only vs remote target), 2 LOW; rc2-chain + MVC clean. **g.7** wrap-hardening (06f5e6d) — scan last-❯→BOTTOM input REGION for the TAIL probe (head rejected: prefix identical across msgs); committed text moves ABOVE ❯ → outside region → no false-staged. **task-s2-j** peer-driven rewind.drive (495e7eb) — correct-by-construction fail-safe: otmux.rewind.drive(View: SELF+IDLE guards, post/verify-picker/navigate-capture-between-keys/select/verify) + hiveMind.agent.rewind(Controller: Tron-auth + DURING_REWIND=the poke-guard, rewind-hold holds ALL sends so no Enter hits the picker); KNOWN select-Enter-stall=first-class fail-safe (Escape out, agent UNHARMED, rc2 Tron-drive/TRUE-FORK, never blind-re-Enter); select uses bare send.raw Enter, NO preceding-ESC (Tron insight). **Whole send saga arc: OTR-1→g.1(kind-branch)→g.4(kind FN)→dup-fix(fccdad8 auto-heal rc2 fall-through)→2fdce8e(verify-commit self-heal)→g.5(matrix)→g.6/g.7(wrap)→j(rewind reuses it all).**
- **★ LIVE DUP FIX — Tron#1 (fccdad8 dev, PO QA-PASS, tester D3/E5 8/8):** all messages duplicated. PO delegated the fix to ME (architect touched production) as expert was wedged 50min. ROOT (found by REPRODUCTION): agent.send unknown-state→auto-heal→inform delivers (OTR-1 honest rc2=staged-ON-PANE), old `if rc==0` let rc2 fall out → `;&` fall-through to queue)→enqueue→idle-drain RE-DELIVERS = 2× (2 prefixes, 1st ^[=claude submit Escape). OTR-1's honest-rc EXPOSED the latent `;&` fall-through; fd-pressure(shell leak)→many unknown-state→'all messages'. FIX: rc2=already-delivered→return, never re-queue; only rc3/rc1 fall through. **DIAGNOSTIC LEARNING: when every code path reads single-invoke but a double is real+reproducible, the bug is in the CALLER/delivery orchestration (a fall-through/re-deliver), NOT the primitive — REPRODUCE to localize, don't just code-read.** [[measure-a-stable-state-not-a-moving-target]]
