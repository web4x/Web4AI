...
- /rewind restores an agent to a previous conversation point. Send '/rewind' to the agent pane, arrow up/down to pick the point, Enter to select. CRITICAL: choose option 1 "Restore code and conversation" — this DROPS everything after the point cleanly. NEVER choose option 2 "Restore conversation" alone or option 4 "Summarize from here" — these SUMMARIZE which DESTROYS detailed context same as compact. TRIGGER: when an agent's prompt is too long / context full — rewind to a known-good checkpoint. F-REWIND: chose option 2 (summarize) on the expert and destroyed trained context — NEVER AGAIN.
