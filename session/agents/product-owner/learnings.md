...
- Token budget calculation (2026-04-22/23): Estimated 5h budget ≈ 400k total tokens (input+output combined). Data: 33% ≈ 130k tokens (expert reading 5500-line hiveMind + writing multi-team resolve). 21% ≈ 84k tokens (subagent 52k + expert+tester ~32k). One 1M context agent at 50% context sends ~500k input tokens per turn — exceeds entire 5h budget in ONE prompt. Practical limit: 2-3 major refactors per 5h window. MUST stagger agents, never run all simultaneously. Check subscription every 10 min via PO shell.
