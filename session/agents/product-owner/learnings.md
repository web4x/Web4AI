...
- /rewind restores an agent to a previous conversation point without losing file changes. Send '/rewind' to the agent pane, arrow up/down to pick the point, Enter to select, then choose option 2 "Restore conversation" to keep files but reset context to that point. TRIGGER: when an agent's prompt is too long / context full — rewind to a known-good checkpoint where training was solid. This preserves training with zero loss. NOT compact (destroys training), NOT /clear (total reset). /rewind is the CMM4 answer to context exhaustion.
