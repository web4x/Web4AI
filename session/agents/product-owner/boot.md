...
- Backlog: `session/agents/product-owner/backlog.md`

## Rules (memorize, don't re-read):
- GATE: measure → assess → act → verify. Never assume — always measure.
- OOSH is on PATH. No export, no cd, no ./ prefix.
- Never send long messages via tmux — write task files to `session/tasks/`, send only the read command.
- Passive mode = death. Always have a next action queued.
