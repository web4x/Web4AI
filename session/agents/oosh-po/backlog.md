...
- [ ] **BUG: fresh `ossh install` produces a polluted (logic-laden) user.env** — u20 (container 4faed70700c9) freshly installed via `ossh install` → broken env: `config list` EMPTY (RC=0), OOSH_MODE empty, OOSH_DIR on prod tree while git=dev. Root cause: install chain (`ossh.install.continue.local` → `config.save`) writes the `43796be` BASH_SOURCE self-anchor + `config.add` source lines; on a symlinked `~/config`→sharedConfig the self-anchor mis-resolves CONFIG_PATH to the symlink dir → cascade. Pollution is BORN AT INSTALL. This is the 3rd source of the #4 env-files violation (install path), beyond config.add + 43796be. **Findings + fix documented in `session/tasks/env-files-pure-state-architecture.md` → "u20 LIVE FINDING".** Fix = #4 core (d45031a, done on main) + install-path must emit pure state + config.validate gate in install + regen u20/WODA.prod. Owner: oosh-expert after T-ENV-PURE green. Tie to #4.
