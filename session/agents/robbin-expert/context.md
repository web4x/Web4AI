...
- OPEN Qs to PO: (a) merge target — 2703628c→8f74dfba (the two real +4981 dups, keep Heartspaces owner) vs 3-way into 3effa1fc (different number — carry both numbers?). (b) real code gap = dedup not firing at 2nd/3rd mint (likely phone entered post-IDENTIFY, or symlink overwrite), NOT normalizePhone — investigate resolve-at-mint path. HOLDING destructive prod merge until confirmed.
