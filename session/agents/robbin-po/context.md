...
- ★ NUMBERING: this is R30.14 (existing sw-auto-update, req refined 2af4839b0) — architect mis-referenced 'R30.33' (TAKEN by deletion-emit); corrected. req to ADD the network-first-shell to R30.14's AC (its first refinement missed the cache-first-shell root cause). Chain: req R30.14 AC-update → architect derive-confirm → expert build → tester gate (left-open deploy → prompt ≤60s no reload).
