...
- ★ KEY HYPOTHESIS (req flag): CHICKEN-AND-EGG — the auto-update code must REACH a client once, then it self-updates. So Tron's hard-reload may be a ONE-TIME bootstrap (mechanism works going forward) OR the built SW has a bug. ARCHITECT root-causing sw.js/CACHE_NAME/version-wiring in build.mjs (is sw.js generated or static per-build?) — DECISIVE. Then: if works → tester GATES forward auto-update (long-open client + new deploy → auto-updates no reload); if bug → expert fixes.
