...
- R30.35 ACTIONS (x/>>): ★ TRON DIAGRAM-GATE — architect writes PUML+SVG of ALL {ADD/DEL/MOD/CONFLICT}×{x/>>/<<}+edge combos + validates vs live (MANY broken per Tron) + DUAL-LINKS; SCENARIO-FIRST (req mints combo units, planner tasks, architect diagrams from units). IMPLEMENT NOTHING until Tron approves the SVG — I SHOW him the SVG first (SendUserFile). Expert HOLDING.
- NEW (Tron): brighter-current-change on up/down nav + count of open changes needing action — req minting (queued after x/>> units), planner tasks, architect designs after the diagram. Scenario-first.
