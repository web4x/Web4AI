...
**NEXT:** await planner's per-task earned-evidence feed (S40 live snapshot + verified WARN owners); tick same-hour. Watch peer progress reports.
...
**T40.1 (7a956c21) CR-BAND MIGRATION (PO bounded job, ACTIVE — awaiting A/B pick):** measured BEFORE = clean QA-Review = Done-block INACTIVE despite 5 open CRs (checklist predates R40.59, no processing-CR sub-step; the 5 CRs are EXTERNAL units, changeRequests not on the task). ★ PO's intent.subStep pointer CAN'T do it (measured): tickSubStep validates vs IN_PROGRESS_SUBSTEPS (excludes 'processing change requests') + TICKS=RESOLVES. Insert-OPEN lives only in intent.reopen. BOTH variants derive 'QA-Review-with-open-CR' (🔁, not approvable = block active): (A) reopen = existing seam, UNTICKS QA-Review (decline-band); (B) keep QA-Review ticked + open CR sub-step = faithful, needs a small expert intent (intent.insertOpenCr). Recommended B w/ expert (0.1, owns task-policy + builds resolveChangeRequest = the tick-to-resolve pair). My migration must land BEFORE expert's resolve action. Checklist-only, never literal/Done, sub-step stays UNTICKED (making block ACTIVE not clearing). Awaiting PO A-vs-B + expert intent shape.
