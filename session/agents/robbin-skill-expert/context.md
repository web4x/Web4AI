...
(A) RETURNING-PROFILED-USER re-gate = NOT a defect, path CORRECT: app.ts:64-82 gates on server MSG.PROFILE.profileCommitted; identity persists localStorage (RawBinClient:32-37 player-id+device-id+keypair); returning same-device re-IDENTIFYs (challenge signed :118) -> server returns committed profile -> rooms. Only re-gate = new-device/cleared-localStorage = INTENDED.
