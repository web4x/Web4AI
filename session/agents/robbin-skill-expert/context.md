...
Built the FAILABLE lint session/tools/lint-rewind-vocabulary.mjs (scans the HAZARD = whole-word banned-term+compounds in the rewind sense, NOT a file list; --self-test proves RED->GREEN; whitelists diff-senses [release/tag, both-ways, readline notice, clear-] + named/quoted term). RED baseline: ENFORCED canon 36 uses -> REPLACED -> GREEN 0 (committed c1381c6e). Canonical law+WHY written into base-skills/agent-rewind.md (Tron verbatim PRESERVED — his quotes already said rewind; only commentary changed); process-canon POINTs to it. Trainer owns role SKILLs (already 0). ANCHOR TIER = 694 raw across 31 agent anchors/learnings = per-agent-on-rewind (NOT a cross-agent sweep). ★ LINT REFINEMENT NEEDED before enforcing the anchor tier: whitelist memory-link SLUGS ([[..-a-rewind]] filenames/identifiers, NOT prose) — the 694 includes slug false-positives. MY OWN anchor/learnings still hold prose+slug uses — I fix mine on my next rewind (careful, slug-preserving). Do NOT run a blind replace on anchors (breaks [[links]]/.md slugs). Awaiting PO call on anchor-tier enforcement.
