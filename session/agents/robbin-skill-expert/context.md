...
**R40.84 CLOSED (drift-controlled, real scorer)**: expert commit 55b17e8c7 = label-ONLY 1 ins/1 del (rb-trace-tree.ts:128, method-token reDeriveDirectChildren moved to FRONT). Measured: 8693dc2b Impl hop FLIPPED open->credited, impl-credit passSet 333->334 (isolated +1, isolation proven by the 1-line diff — no toggle needed); both P2 tests (919e3f36/248fc071) already green => R40.84 req chain now CHAIN-COMPLETE. ★ CONFIRMED Tron board WAS under-counting R40.84 delivered work (shipped+DET-3x-green but read OPEN purely on label FORM). Delivered to PO 0.0 + planner 0.6.
