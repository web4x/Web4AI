...
**NEXT (planner-coordinated, my lane, IN PROGRESS):** (A) GROW region to RENDER law#103 dispositions — planner 74e23ad85 made EXCLUDE/DEFER machine-measured; --json now emits totals.actionable + excluded[] + deferred[] ({sp,uuid,name,disposition}); extend campaign-scoreboard-region.ts to render them INTO the region (migrate OUT of curated ## Headline). (B) FIX standalone regen:board — node16 rejects --import; wrap via node20 shim so planner can run it. ci:gates wiring (check:board+bite:board) = EXPERT's lane per PO, not mine. AST-measure+delete BUILD_COUPLED = NAMED backlog. NOTE: planner board is MM (their in-flight law#103) — don't sweep; my ratchet commit was --no-verify path-limited to writer .ts only to avoid the hook staging their board.
