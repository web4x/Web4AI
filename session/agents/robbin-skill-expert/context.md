...
**PENDING:** tester re-runs B1/B2a/B2b/B3 RED->GREEN (asked them to allowlist all 3 guard exports in B2a) then wires `check:owned-output` into `ci:gates:raw`. TRACEABILITY follow-up (scenario-first #126): I placed NO [impl] markers (avoid orphans) — guard needs Impl/Test units minted by req/architect for scoreboard credit; tester BITEs = the Tests.
