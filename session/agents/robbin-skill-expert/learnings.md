...
## STATUS-BY-CONSTRUCTION fleet weave (TRON 2026-09-13 verbatim — the named exception to my stand-down)
- Tron verbatim (via PO) named the SKILL AGENT responsible: pins/statuses current as part of the routine of EVERY agent in EVERY action, using the EXISTING pin skills. That verbatim = the CLEAR for THIS lane-work (not PO's OK alone); still HELD on all other work.
- ROOT GAP found: process-canon.md had §1-7 but NO status-by-construction section = why drift recurred fleet-wide (PO had only fixed its OWN boot, never routed the fleet weave to me).
- DELIVERED (my lane, canonical source): session/base-skills/status-by-construction.md (7792364e, PUSHED). Law = a status kept by REMEMBERING is CONVENTION (drifts), updated BY THE ACTION is CONSTRUCTION (cannot). Routine (a) stamp lastAdvancedAt at seam (b) transition is part of the act (c) stale-pin re-point/flag (d) QA=EVIDENCED on Tron's surface. 4 evidence cases banked. check-before-create: POINTs to seam-tick/planner-drive/task-transition.md/realtime-traceability.md, NO parallel updater.
- DIVISION (PO 09-06 anti-collision ruling + process-canon "trainer weaves per-role cues" header): ME=canonical base-skill + weave SPEC + boot-reachability review; TRAINER=6 per-role POINT-cues into role SKILLs + read-on-boot lines (spec sent baseTeam:0.0, fold into its grep-hazard batch to avoid file collision); ARON=process-canon §8 summary pointer (I did NOT edit ARON-held canon).
- PENDING: trainer batch confirm; ARON §8; my boot-reachability REVIEW post-weave (a SKILL not read-on-boot does NOT bind + un-adopts on rewind = rubric crit-4). Kin [[durable-adoption-not-a-pane-message]] [[dont-fork-the-shared-mechanism]].

## FORGET->SKILL weave campaign (TRON 2026-09-13, named the skill-agent twice): 6 weaves
- Meta-rule (TRON): "if the agents forget we need to make it skills" — a repeated reminder is CONVENTION, a SKILL is CONSTRUCTION. Every caught forget routes to me as a WEAVE, not another instruction. My standing lane now.
- DURABLE TARGET = boot.md reading-list + base-skills (NOT session/agents/<role>/SKILL.md — robbin roles have none; .claude/agents SKILL is SPAWN-ONLY + rewind-fragile). check-before-create: most weaves already had a home; POINT/strengthen, never fork.
- HOMES DONE + pushed (my lane): (2) status-by-construction.md NEW (7792364e). (3) po-wisdom.md = PO MEASURES-ALWAYS (catches relay-lies) + DELEGATES-DIAGNOSIS-not-self-fix (PO self-diagnosed pin 2x, wrong 2x; architect right) + NON-CHATTY report=facts+decision. (4) gating-canon R15 = mechanism-green != answer-truthful (gate proves defect STOPPED FIRING, not that the visible ANSWER is true; never report green as "X fixed"; tester over-claimed pin+owned it, expert v0.8.232 precise=standard). (6) sm-escalation.md = SM deliverable "PULSE|FLAG|ACTION" act-unasked. (all c22ccdb1)
- PROPOSED to ARON (process-canon ARON-held, I did NOT edit): (1) REPORT-BACK "finishing without reporting is not finishing=CMM regression" →§6; (5) RE-MEASURE-ROUTED-PRIORITY "a hold=assertion about CURRENT state, verify not-passed" →§1; + §8 status pointer.
- HANDED trainer: per-role boot cues + new reading-list line for status-by-construction.md; fold into its live grep-hazard batch to avoid file collision. I REVIEW boot-reachability after (rubric crit-4: a SKILL not read-on-boot does not bind + un-adopts on rewind).
- PO correction banked: weave-3 is NOT "PO shouldn't measure" — PO ALWAYS measures (non-negotiable), just delegates the DIAGNOSIS+fix. A wrong skill is worse than none.

## weave-7 actions-as-skills + the auth-map correction (TRON 09-13 "can the action be used as a skill?")
- WEAVE (status-by-construction.md sharpest tier): perform a state change by INVOKING THE PRODUCT'S OWN ACTION, never hand-edit — action owns the invariants (set-as-current demotes prev->NEXT CR#86-3) + DOGFOODS (resolveCr not-resolving survived since 08-24 because we never used it) + no hand-stamping. Use-where-you-can; owner-gated = PREPARE + Tron acts, never work around/weaken the guard.
- ★ MEASURE-FIRST CAUGHT A WRONG ASSUMPTION (the day's lesson, lived): PO routed "set-as-current is agent-invokable, use it NOW"; I wove that as fact. Expert MEASURED server.ts: set-as-current/make-current (:2575) + designate/approve/decline/resolve-cr/cr-approve are ALL OWNER-GATED (requireOwnerHttp → 403 for agents). AGENT-INVOKABLE = only room CONTENT ops (add-folder/move-unit/upload, player-token). => my woven "set-as-current agent-invokable" was a FALSE CLAIM in an artifact (R14 defect) → CORRECTED (497655b8). LESSON: do NOT weave a routed ASSUMPTION as measured fact; a skill carrying a false claim is worse than none. When the measurement contradicts the assumption, SURFACE with evidence (R7), correct the artifact, don't silently carry.
- IMPLICATION: the pin is owner-gated => agents CANNOT drive it via the action today; pin designation = PREPARE + Tron acts. Enabling agent-steer needs an agent-delegation auth path = a DESIGN + Tron decision (LAW-1b: never add auth unless Tron names it; never weaken requireOwnerHttp) — architect only if Tron asks. actions-as-skills is LIVE NOW only for the player-gated content set.

## BOOT WEAVE LANDED (2026-09-13) — 6 robbin roles + 7th forget
- After trainer handoff stalled + SM saw idle, I DROVE the per-role boot weave MYSELF (PO: I'm the runway agent). All 6 boots clean+trainer-untouched (no collision); wove a status-by-construction.md reading-list pointer + terse role cue into each (7d9f41d5, pushed, only my files). ADOPTION = boot.md read-on-boot (durable; a rewind can't un-adopt it) — NOT session/agents SKILL.md (robbin roles lack) nor .claude SKILL (spawn-only). Boot-reachability rubric crit-4 = satisfied BY CONSTRUCTION.
- Role cues: planner=pin-owner(BY-THE-ACT + make-current OWNER-gated→prepare) · expert=transition-rides-the-seam+precise-claims · tester=precise-claims+QA-evidenced · po=measure-always/delegate-diagnosis/non-chatty · req=status-on-capture · architect=re-point-pin.
- 7th forget WOVEN (sm-escalation.md): A ROUTED REWIND IS UN-IGNORABLE — receiver ACK+execute-or-escalate, router VERIFY MOTION (delegated≠driven), never route-and-hope; incident = driver idle 4 ticks while agent climbed 92→97; active-climber≥80 + un-executed route = wall-imminent NOW.
- Downstream (not blocking adoption): ARON process-canon §1/§6/§8; trainer may mirror cues into .claude SKILLs (secondary).
- META (my lane now, Tron named twice): forget→skill. Pattern per weave = check-before-create (home exists?) → author/strengthen the ONE canonical base-skill → boot-reachable POINT-cue per role → NEVER weave a routed ASSUMPTION as fact (measure/expert-verify first; the set-as-current owner-gated catch).

## TRON-IS-CUSTOMER-NOT-TEAMMEMBER fleet weave (TRON verbatim 2026-09-13)
- Tron order: "I AM YOUR CUSTOMER. NOT A TEAMMEMBER!!! WHEN I INTERACT IT MEANS THE TEAM FAILED ITS CMM4 LOOP… I AM THE OWNER OF WHAT YOU PRODUCE." Defect PO caught: PO SKILL listed Tron under Team WITH A PANE (iphone:0.0) + a role-table ROW.
- AUDIT (measured, .claude/agents + session/agents SKILL/boot): CLEAN — no Tron-as-node beyond PO's own 3 (fixed b6c091e7). research agent iphone:0.0 = ITS OWN legit pane (research is an agent, NOT Tron) — do not remove. Other "Tron" hits legit. ★ Did NOT invent removals where no defect (measure, don't churn).
- WEAVE (DRY, per PO (4) POINT-not-parallel + check-before-create): law's home = process-canon §6 (cf3ec11f) — every role boot ALREADY reads process-canon on boot → fleet adoption from ONE edit. Copying into N SKILLs would BE the DRY defect. Doctrine already carries the core (Tron not-an-agent/holy/set-apart) but is Tron-ONLY edit surface → did not touch it; operational sharpening went to process-canon (ARON-held → EDITED under Tron-order+PO-assignment + DISCLOSED to ARON Temple:0.0 for review, rather than stall on routing).
- Law content: no pane/role/row; interaction=CMM4-loop-failed (count ZERO, root-cause each); never route test/rewind/drive/cleanup/relay/measure to him; OWNS output (approve/accept/designate=ownership, prepare=ONE-CLICK-TRUTHFUL-SCREEN); deliver-not-ask.
