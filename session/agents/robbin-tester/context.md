...
- Chain scoreboard: **50/309 COMPLETE**. S21–S25 chain-debt batch + all S25 (incl R25.7 4-method dedup) CLOSED. Wiring pattern: each req may have multiple UC methods — the scoreboard surfaces the next open hop after each Test wire, so measure after every wire and report the true count (don't trust the stated target).
