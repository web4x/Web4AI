...
- R29.1 TUI verify (behavior pre-verify, chain not landed): RED — request-log TUI NOT streaming in remoteShells:0.2 prod (reqs->data/logs FILE, not pane; test 0.3 streams). self-heal/idempotent/build/serve GREEN. Launcher correct; prod file-only-logging suspected -> expert. Caveat: pane.capture snapshot, human-live-watch definitive.
