...
## NEXT MAIN GOAL (PO): S24 — formalize planner-drive.ts/objectVerb.ts/generate-sprint-md.ts/trace-cli.ts as OOSH-like TS skills (self-documenting, method-dispatch, Tab-completable). Design Object.verb API (Chain.scoreboard, Sprint.generate, Pin.focus). Study OOSH pattern (components/OOSH/). Coordinate: planner 0.6 (heaviest user of planner-drive focus/hop/gate/getThreeSlots + generate-sprint-md round-trip --check, offered to brief), skill-expert (tool owner), req (requirements, CMM4 #18 req->architect->expert->tester). NOT yet started.
