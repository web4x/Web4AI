...
**How to apply:** (1) Know the schema exactly before mutating: in this graph `ownerIor` is a TOP-LEVEL unit field, NOT `model.ownerIor`; a mutation to the wrong nesting silently no-ops the real edge. (2) A delete-and-repoint migration must repoint EVERY inbound edge to the deleted node — forward (Class.methods) AND back (Method.ownerIor) — and the completeness gate must scan ALL slots incl ownerIor, not just the forward UC refs I checked. (3) NEVER verify by reading the field you just wrote — read the independent/canonical field, or have a different agent/tool measure; a self-scoped check confirms your intent, not reality. Apply your own hardest-won lessons to your OWN work first — that's where they lapse.
